...
public class PedidoService {
	...
	public boolean agregarPedido(Pedido pedido) {
		if (!existePedido(pedido)) {
			try (PrintStream ps = new PrintStream(new FileOutputStream(RUTA, true))) {
				ps.println(pedido.getNumero() + SEPARADOR + pedido.getProducto() + SEPARADOR + pedido.getPrecio());
				return true;
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	private void volcarPedidos(List<Pedido> pedidos) {
		try (PrintStream ps = new PrintStream(new FileOutputStream(RUTA, false))) {
			pedidos.forEach(p -> ps.println(p.getNumero() + SEPARADOR + p.getProducto() + SEPARADOR + p.getPrecio()));
		...
		try (BufferedReader bf = new BufferedReader(new FileReader(RUTA))) {
			...
			while ((linea = bf.readLine()) != null) {
				...
				if (numeroPedido == Integer.parseInt(datos[0])) {
					pedido = new Pedido(Integer.parseInt(datos[0]), datos[1], Double.parseDouble(datos[2]));
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public List<Pedido> pedidosPendientes() {
		...
		try (BufferedReader bf = new BufferedReader(new FileReader(RUTA))) {
			...
			while ((linea = bf.readLine()) != null) {
				// transformamos la línea en un array de atributos del pedido
				String[] datos = linea.split(SEPARADOR);
				lista.add(new Pedido(Integer.parseInt(datos[0]), datos[1], Double.parseDouble(datos[2])));
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
